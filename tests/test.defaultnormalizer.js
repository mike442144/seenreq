...
assert.equal(true, seen.normalize("https://www.google.com.hk/?gfe_rd=cr&ei=qg5QVYyVBcrC8Afz6ICoDw&gws_rd=ssl&safe=strict&q=how+to+test+url+duplicate").update);
...
assert.equal(true, seen.normalize(opt).update);
...
assert.equal("GET http://www.google.com/\r\n", seen.normalize(opt).sign);
assert.equal(false, seen.normalize(opt).update);
